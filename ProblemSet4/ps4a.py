...
def getWordScore(word, n):
    ...
    return sum([SCRABBLE_LETTER_VALUES[l] for l in word]) * len(word) +\
        50 * max(0, len(word) - n + 1)
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    myHand = hand.copy()
    while myHand and sum(myHand.values()) != 0:
        # Display the hand
        handDisp = ""
        ...
        # Otherwise (the input is not a single period):
            ...
            if not(isValidWord(word, myHand, wordList)):
                # Reject invalid word
